...
//            // שיטת המימוש שתמיד תחזיר k מרכזים בדיוק, בהתבסס על אלגוריתם של Gonzalez
//            // אלגוריתם זה נחשב לגרסה של Hochbaum & Shmoys ומהווה קירוב טוב בפקטור 2
//            // אנחנו נמצא את הצמתים בצורה הבאה:
//            // 1. לוקחים צומת ראשון כלשהו כמרכז
//            // 2. בוחרים את הצומת הכי רחוק מכל מרכז קיים
//            // 3. מוסיפים אותו למרכזים
//            // 4. חוזרים על 2-3 עד שנגיע ל-k מרכזים

//            var nodeIds = _graph.Nodes.Keys.ToList();
//            var centers = new List<long>();
//            var selectedCenters = new HashSet<long>();

//            // בחירת צומת ראשון - יכול להיות ראשון או באמצע לפי בחירה
//            long firstCenter = nodeIds[0];
//            centers.Add(firstCenter);
//            selectedCenters.Add(firstCenter);

//            // הוספת k-1 המרכזים הנותרים
//            for (int i = 1; i < k && i < nodeIds.Count; i++)
//            {
//                long farthestNode = -1;
//                double maxMinDistance = -1;

//                // מציאת הצומת שהמרחק המינימלי שלו מהמרכזים הקיימים הוא הגדול ביותר
//                foreach (var node in nodeIds)
//                {
//                    if (selectedCenters.Contains(node))
//                        continue;
...
//                if (farthestNode != -1)
//                {
//                    centers.Add(farthestNode);
//                    selectedCenters.Add(farthestNode);
//                }
//                else
//                {
//                    // אם לא נמצא צומת נוסף (למשל אם כל הצמתים כבר נבחרו)
//                    break;
//                }
//            }

//            // חישוב הרדיוס המקסימלי
//            double maxRadius = 0;
//            foreach (var node in nodeIds)
//            {
...
//                }
//                maxRadius = Math.Max(maxRadius, minDistToCenter);
//            }

//            return (centers, maxRadius);
//        }
